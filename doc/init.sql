...
CREATE TABLE `lease_record`  (
  ...
  `lease_state` tinyint(3) UNSIGNED NOT NULL DEFAULT 0 COMMENT '行程状态：1-未开始，2-行程中，3-已结束，4-租赁失败，5-租赁超时，6-丢失结束，7-超时结束，8-减免结束，9-取消租赁，10-预开始，11-雨伞未注册，12-废弃，13-售出结束',
  `trade_state` tinyint(3) UNSIGNED NOT NULL DEFAULT 0 COMMENT '交易状态：0-待支付, 1-预付中, 2-已预付, 3-支付中, 4-已支付, 5-退款中, 6-退款完成',
...
CREATE TABLE `trade_flow`  (
  ...
  `state` tinyint(3) UNSIGNED NOT NULL DEFAULT 0 COMMENT '交易状态：0-待支付, 1-预付中, 2-已预付, 3-支付中, 4-已支付, 5-退款中, 6-退款完成, 7-已关闭',
...
CREATE TABLE `user`  (
  ...
  `tel_number` char(11) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL DEFAULT '' COMMENT '手机号',
  `create_time` timestamp(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间',
  `update_time` timestamp(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3) COMMENT '更新时间',
  PRIMARY KEY (`id`) USING BTREE,
