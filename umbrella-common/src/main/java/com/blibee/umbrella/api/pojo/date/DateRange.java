...
import com.blibee.umbrella.common.util.Safes;
...
final public class DateRange implements Iterable<ShortDate>,
    ...
    public static DateRange create(String json) {
        Preconditions.checkArgument(StringUtils.hasText(json));
        json = json.replace("[", "");
        json = json.replace("]", "");
        List<String> strings = splitter.splitToList(json);
        Preconditions.checkArgument(strings.size() == 2);
        ShortDate from = ShortDate.create(Safes.of(strings.get(0)).trim());
        ShortDate to = ShortDate.create(Safes.of(strings.get(1)).trim());
